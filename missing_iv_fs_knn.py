...
def fill_missing_values(data_train, data_test, type2, k_neighbours=3, meth=None, train=True):
    if type2 == 1:
        ...
        data_filling_missings = data_test.copy()  # tablica danych z uzupełnionymi wartościami
        ...
        for obj_num in range(marks_missing.shape[0]):
            if marks_missing[obj_num][0] == 1:
                ...
                if train:
                    new_X_train = data_filling_missings.copy()
                else:
                    new_X_train = data_train.copy()
                new_y = y_train.copy()
...
if __name__ == '__main__':
                        ...
                        for meth in meths:
                            for prec in precedences:
                                ...
                                for i in range(attempts):
                                    ...
                                    print('order', ord)
                                    print('prec', prec)
                                    print('aggregatin_prec', p_agg.aggregation_name())
                                    print('attempt ', i)
                                    print('method filling missings', miss_rate, type2, meth)
                                    ...
                                    print(X_train.shape)
                                    X_train = fill_missing_values(X_train, X_train, type2=type2, k_neighbours=k,
                                                                  meth=meth)
                                    X_test = fill_missing_values(X_train, X_test, type2=type2, k_neighbours=k,
                                                                 meth=meth,
                                                                 train=False)
                                ...
                                # print(accuracy)
                                # print(accuracy[-1])
                                print(np.mean(accuracy[-1]), np.max(accuracy))
                                print('sensitivity=')
                                # print(sensitivity)
                                # print(sensitivity[-1])
                                ...
                                average = pd.DataFrame(
                                    {'accuracy': np.mean(accuracy), 'sensitivity': np.mean(sensitivity),
                                     ...
                                     'precedence': prec, 'aggr_prec': p_agg.aggregation_name(),
                                ...
                                concatenated.to_excel(
                                    path_prefix + 'result/part/' + f_mark + data_slice + 'part_Miss_Full' + str(
                                        miss_rate) + aggg.aggregation_name() + str(
                                        type2) + today + meth + datetime.today().strftime(
                                        "%d-%b-%y_%H.%M") + '.xlsx')
                                # + dec_aggr+ prec+ meth
    ...
    filename = path_prefix + 'result/' + f_mark + data_slice + '_Miss_Full' + str(
        miss_rate) + aggg.aggregation_name() + str(type2) + today + meth + datetime.today().strftime(
        "%d-%b-%y_%H.%M") + '.xlsx'
