...
class A1(Aggregation):
    def aggregation_function(self, fuzzy_sets, order='possible'):
        if type(fuzzy_sets[0]) != IntervalValuedFuzzySet:
            fs = np.array([IntervalValuedFuzzySet.from_numpy(f, order) for f in fuzzy_sets])
            fuzzy_sets = fs
        if type(fuzzy_sets[0]) == IntervalValuedFuzzySet:
            if len(fuzzy_sets) > 2:
    ...
    def aggregation_function(self, fuzzy_sets, order='possible'):
        ...
        if type(fuzzy_sets[0]) == IntervalValuedFuzzySet:
            ...
            if fuzzy_sets[0] == ones and fuzzy_sets[1] == ones:
                ...
                if self.agr == 'g':
                    # print("\n",fs)
                    if CMP.check(fs[1][1], 0, '==') or CMP.check(fs[0][1], 0, "=="):
                        b = 0
                    else:
    ...
    def aggregation_function(self, fuzzy_sets, order='possible'):
        ...
        if type(fuzzy_sets[0]) == IntervalValuedFuzzySet:
            ...
            if fuzzy_sets[0] == zeros and fuzzy_sets[1] == zeros:
                ...
                b = 0
                if self.agr == 'g':
                    if CMP.check(fs[0][0], 0, '==') or CMP.check(fs[1][0], 0, "=="):
                        b = 0
                    else:
